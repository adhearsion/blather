...
describe Blather::Stanza::PubSub::Subscription do
  ...
  it 'ensures an subscription node is present on create' do
    ...
    subscription.xpath('//ns:pubsub/ns:subscription', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
  end

  it 'ensures an subscription node exists when calling #subscription_node' do
    subscription = Blather::Stanza::PubSub::Subscription.new :set, 'host', 'node', 'jid', 'subid', :none
    subscription.pubsub.remove_children :subscription
    subscription.xpath('//ns:pubsub/ns:subscription', :ns => Blather::Stanza::PubSub.registered_ns).should be_empty
  ...
  it 'has a node attribute' do
    subscription = Blather::Stanza::PubSub::Subscription.new :set, 'host', 'node-name', 'jid', 'subid', :none
    subscription.xpath('//ns:pubsub/ns:subscription[@node="node-name"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
    subscription.node.should == 'node-name'

    subscription.node = 'new-node'
    subscription.xpath('//ns:pubsub/ns:subscription[@node="new-node"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
    subscription.node.should == 'new-node'
  end

  it 'has a jid attribute' do
    subscription = Blather::Stanza::PubSub::Subscription.new :set, 'host', 'node-name', 'jid', 'subid', :none
    subscription.xpath('//ns:pubsub/ns:subscription[@jid="jid"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
    subscription.jid.should == Blather::JID.new('jid')

    subscription.jid = Blather::JID.new('n@d/r')
    subscription.xpath('//ns:pubsub/ns:subscription[@jid="n@d/r"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
    subscription.jid.should == Blather::JID.new('n@d/r')
  end

  it 'has a subid attribute' do
    subscription = Blather::Stanza::PubSub::Subscription.new :set, 'host', 'node-name', 'jid', 'subid', :none
    subscription.xpath('//ns:pubsub/ns:subscription[@subid="subid"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
    subscription.subid.should == 'subid'

    subscription.subid = 'new-subid'
    subscription.xpath('//ns:pubsub/ns:subscription[@subid="new-subid"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
    subscription.subid.should == 'new-subid'
  end

  it 'has a subscription attribute' do
    subscription = Blather::Stanza::PubSub::Subscription.new :set, 'host', 'node-name', 'jid', 'subid', :none
    subscription.xpath('//ns:pubsub/ns:subscription[@subscription="none"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
    subscription.subscription.should == :none

    subscription.subscription = :pending
    subscription.xpath('//ns:pubsub/ns:subscription[@subscription="pending"]', :ns => Blather::Stanza::PubSub.registered_ns).should_not be_empty
