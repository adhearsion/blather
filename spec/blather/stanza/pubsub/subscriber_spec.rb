...
describe Blather::Stanza::PubSub::Subscriber do
  it 'can request a subscription' do
    ...
    sub.to.must_equal Blather::JID.new('host.name')
    sub.set?.must_equal true
    sub.find('//ns:pubsub/ns:subscription[@node="node_name" and @jid="j@i.d"]', :ns => Blather::Stanza::PubSub.registered_ns).wont_be_empty
  end
  ...
  it 'can request an unsubscribe' do
    ...
    sub.to.must_equal Blather::JID.new('host.name')
    sub.set?.must_equal true
    sub.find_first('//ns:pubsub/ns:unsubscribe[@node="node_name" and @jid="j@i.d"]', :ns => Blather::Stanza::PubSub.registered_ns).wont_be_nil
  end
  ...
  it 'can request an unsubscribe with a subscription id' do
    ...
    sub.to.must_equal Blather::JID.new('host.name')
    sub.set?.must_equal true
    sub.find_first('//ns:pubsub/ns:unsubscribe[@node="node_name" and @jid="j@i.d" and @subid="subid"]', :ns => Blather::Stanza::PubSub.registered_ns).wont_be_nil
  end
  ...
  it 'knows the values of a subscription' do
    ...
    stanza.subscription.must_equal({
      ...
      :jid          => Blather::JID.new('francisco@denmark.lit'),
