...
describe Blather::Stanza::PubSub::Purge do
  it 'registers itself' do
    Blather::XMPPNode.class_from_registration(:purge, 'http://jabber.org/protocol/pubsub#owner').must_equal Blather::Stanza::PubSub::Purge
  end

  it 'can be imported' do
    Blather::XMPPNode.import(parse_stanza(<<-NODE).root).must_be_instance_of Blather::Stanza::PubSub::Purge
    <iq type='set'
  ...
  it 'ensures an purge node is present on create' do
    purge = Blather::Stanza::PubSub::Purge.new
  ...
  it 'sets the host if requested' do
    purge = Blather::Stanza::PubSub::Purge.new :set, 'pubsub.jabber.local'
    purge.to.must_equal Blather::JID.new('pubsub.jabber.local')
  end

  it 'sets the node' do
    purge = Blather::Stanza::PubSub::Purge.new :set, 'host', 'node-name'
