...
module Blather
  describe 'Blather::XMPPNode' do
    ...
    it 'provides an attribute_reader with converstion' do
      foo = Class.new(XMPPNode) { attribute_reader :bar, :call => :to_sym }.new
      foo.must_respond_to :bar
      foo.bar.must_be_nil
      foo[:bar] = 'baz'
      foo.bar.must_equal :baz
    ...
    it 'provides a content reader' do
      foo = Class.new(XMPPNode) { content_attr_reader :bar }.new('foo')
      foo << (bar = XMPPNode.new('bar', foo.document))
      bar.content = 'baz'
      foo.must_respond_to :bar
      foo.bar.must_equal 'baz'
    end
    ...
    it 'provides a content reader that converts the value' do
      foo = Class.new(XMPPNode) { content_attr_reader :bar, :to_sym }.new('foo')
      foo << (bar = XMPPNode.new('bar', foo.document))
      bar.content = 'baz'
      foo.must_respond_to :bar
      foo.bar.must_equal :baz
    ...
    it 'provides a content reader with a different node' do
      foo = Class.new(XMPPNode) { content_attr_reader :bar, nil, :fiz }.new('foo')
      foo << (fiz = XMPPNode.new('fiz', foo.document))
      fiz.content = 'baz'
      foo.must_respond_to :bar
      foo.bar.must_equal 'baz'
    end
    ...
    it 'provides a content writer' do
      foo = Class.new(XMPPNode) { content_attr_writer :bar }.new('foo')
      foo.must_respond_to :bar=
      foo.bar = 'baz'
    ...
    it 'provides a content writer with a different node' do
      foo = Class.new(XMPPNode) { content_attr_writer :bar, :fiz }.new('foo')
      foo.must_respond_to :bar=
      foo.bar = 'baz'
    ...
    it 'provides a content accessor' do
      foo = Class.new(XMPPNode) { content_attr_accessor :bar }.new('foo')
      foo << (bar = XMPPNode.new('bar', foo.document))
      foo.must_respond_to :bar=
      foo.must_respond_to :bar
      foo.bar = 'baz'
    ...
    it 'provides a content accessor with conversion' do
      foo = Class.new(XMPPNode) { content_attr_accessor :bar, :to_sym }.new('foo')
      foo << (bar = XMPPNode.new('bar', foo.document))
      foo.must_respond_to :bar=
      foo.must_respond_to :bar
      foo.bar = 'baz'
