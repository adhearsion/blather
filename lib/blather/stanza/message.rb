...
class Stanza
  ...
  #                    session. Typically a receiving client will present message
  #                    of type `chat` in an interface that enables one-to-one
  #                    chat between the two parties, including an appropriate
  #                    conversation history.
  # * `:error`      -- The message is generated by an entity that experiences an
  #                    error in processing a message received from another
  #                    entity. A client that receives a message of type `error`
  #                    SHOULD present an appropriate interface informing the
  #                    sender of the nature of the error.
  ...
  #                    `groupchat` in an interface that enables many-to-many chat
  #                    between the parties, including a roster of parties in the
  #                    chatroom and an appropriate conversation history.
  # * `:headline`   -- The message provides an alert, a notification, or other
  #                    information to which no reply is expected (e.g., news
  #                    headlines, sports updates, near-real-time market data, and
  #                    syndicated content). Because no reply to the message is
  #                    expected, typically a receiving client will present a
  #                    message of type "headline" in an interface that
  #                    appropriately differentiates the message from standalone
  #                    messages, chat messages, or groupchat messages (e.g., by
  #                    not providing the recipient with the ability to reply).
  ...
  #                    to reply, but without a conversation history. The default
